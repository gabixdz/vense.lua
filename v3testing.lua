...
local function createHealthBar()
    ...
    Connections.HealthBar = humanoid.HealthChanged:Connect(function(hp)
        -- Clamp to 0 minimum, ignore negative values from death damage
        local safeHp = math.max(hp, 0)
        updateBar(safeHp, humanoid.MaxHealth)
    end)

    -- Death: show clean "dead" state, stop danger pulse, lock bar at 0
...
-- ── PARTICLES ─────────────────────────────────────────────────────────────────
-- ══════════════════════════════════════════════════════════════════════════════
local ParticleOrbs = {}   -- holds the orb Part instances
local ParticleGui  = nil  -- the ScreenGui holding all BillboardGuis
...
local function destroyParticles()
    ...
    ParticleOrbs = {}
    if ParticleGui then ParticleGui:Destroy() ParticleGui = nil end
end
...
local function createParticles()
    ...
    local NUM_ORBS   = 8      -- number of glowing balls
    local RADIUS     = 3.5    -- orbit radius in studs
    local ORB_SIZE   = 0.35   -- diameter of each orb
    local SPEED      = 1.2    -- orbit rotations per second
    local BOB_HEIGHT = 1.2    -- how much they bob up/down
    local BOB_SPEED  = 0.8    -- bob oscillation speed

    local col = ColorStates.ParticlesColor

    -- Create each orb as a Part with a BillboardGui glow circle on top
    for i = 1, NUM_ORBS do
        local orb = Instance.new("Part")
        orb.Name        = "VenseOrb_" .. i
        ...
        -- Sphere mesh so it looks like a proper ball
        local mesh = Instance.new("SpecialMesh")
        mesh.MeshType = Enum.MeshType.Sphere
        mesh.Parent   = orb

        -- Soft glow halo via BillboardGui
        ...
        glow.Image                 = "rbxassetid://6407871923"  -- soft radial gradient, always available
    ...
    -- Animate all orbs each heartbeat
    local startTime = tick()
    Connections.Particles = RunService.Heartbeat:Connect(function()
        local char2 = getChar()
        if not char2 or not Features.Particles then
            destroyParticles()
            return
        end
        ...
        for i, orb in ipairs(ParticleOrbs) do
            ...
            -- Evenly spread orbs around the full circle
            local angleOffset = (i - 1) / NUM_ORBS * math.pi * 2
            local angle       = t * SPEED * math.pi * 2 + angleOffset

            -- Each orb bobs at a slightly different phase so they ripple
            local bobPhase  = (i - 1) / NUM_ORBS * math.pi * 2
            local bobY      = math.sin(t * BOB_SPEED * math.pi * 2 + bobPhase) * BOB_HEIGHT

            local x = base.X + math.cos(angle) * RADIUS
            local y = base.Y + 1 + bobY          -- +1 = sit at torso height
            local z = base.Z + math.sin(angle) * RADIUS

            orb.CFrame = CFrame.new(x, y, z)

            -- Pulse size slightly for life-like feel
            local pulse  = 1 + math.sin(t * 3 + bobPhase) * 0.15
            local sz     = ORB_SIZE * pulse
            orb.Size     = Vector3.new(sz, sz, sz)

            -- Rainbow: cycle each orb hue offset from the next
            if ColorStates.ParticlesRainbow then
                local hue = ((t * 0.4) + (i - 1) / NUM_ORBS) % 1
                local rc  = Color3.fromHSV(hue, 1, 1)
                orb.Color = rc
                -- Update glow colour too
                local bb2 = orb:FindFirstChildOfClass("BillboardGui")
                if bb2 then
                    local img = bb2:FindFirstChildOfClass("ImageLabel")
                    if img then img.ImageColor3 = rc end
                end
            end
        end
    end)
...
local function openParticlesPicker()
    createColorPicker("particles color", "ParticlesColorPicker", function(preset)
        ColorStates.ParticlesColor   = preset.color
        ColorStates.ParticlesNeon    = preset.neon
        ColorStates.ParticlesRainbow = preset.rainbow
        stopRainbowKey("particles")
        if Features.Particles then
            -- Recolor existing orbs live
            for _, orb in pairs(ParticleOrbs) do
                if orb and orb.Parent then
                    ...
                    if bb2 then
                        local img = bb2:FindFirstChildOfClass("ImageLabel")
                        if img then img.ImageColor3 = preset.color end
                    end
                end
            end
        end
